...
def create_figure():
    ...
    for cx, name, what, deliver in increments:
        # header
        draw_box(ax, cx, 9.0, 3.45, 1.0, name, what, fc=COLORS["ink"],
                 ec=COLORS["ink"], tc="white", fs=12.5, sub_fs=9.8)
        ...
        for py, plabel, fc, ec in phases:
            draw_box(ax, cx, py, 2.95, 0.6, plabel, None, fc=fc, ec=ec, tc=ec, fs=12)
        ...
        ax.annotate("", xy=(cx - 1.62, 7.55), xytext=(cx - 1.62, 5.45), zorder=2,
                    arrowprops=dict(arrowstyle="-|>", color=COLORS["muted"], lw=1.1,
                                    ...
                                    linestyle=(0, (3, 2))))
        ax.text(cx - 2.08, 6.5, "refine", rotation=90, ha="center", va="center",
                fontsize=9, color=COLORS["muted"], style="italic")
        # deliverable
        draw_arrow(ax, (cx, 5.15), (cx, 4.55), lw=1.3)
        draw_box(ax, cx, 4.1, 3.45, 0.95, "Deliverable", deliver,
                 fc=COLORS["box"], ec=COLORS["muted"], fs=11, sub_fs=9.5)
