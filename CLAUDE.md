...
3. **No Fabricated Data:** Never hallucinate experimental results. The `[X.XX%]` placeholder convention applies to **this thesis's own empirical values** — anything that requires an actual training/evaluation run (results, chapters 4–6) or an unset study-design parameter (e.g. panel sample size `[N]`). Leave those bracketed until the run/decision produces them.
