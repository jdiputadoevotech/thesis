...
- **Page format when a page target is given:** the paper uses **A4 paper size, 1-inch margins on all sides except the left, which is 1.5 inches, double spacing** (12pt body). Size prose to that layout — budget ~250 words per page (e.g. a 15-page target ≈ 3,750 words). Convert any page count to a word budget on this basis before drafting.
...
3. **No Fabricated Data:** Never hallucinate experimental results. The `[X.XX%]`/`[N]` placeholder convention applies **only to values a run or decision has not yet produced** — a training/evaluation result not yet measured, or an unset study-design parameter. Where a real value now exists, use it (the introduction/methodology drafts and their literature-reported stats are already filled — do not re-bracket a value that is in the drafts). The bar is simply: never invent a number no run has produced.
   - **Exception — literature-reported metrics (esp. Ch2 RRL):** figures *reported by cited papers* are not our findings and are not placeholders to defend. Fill them with the verified value from `literature/references.md` (the single source of truth — the "Key takeaways" column records these). If a needed figure isn't yet in `references.md`, verify it against the source and add it there first, then cite. Do not guess a literature value.
4. **No Repetition — Say It Once (applies to every chapter, draft, slide, and script):** An advisor flagged in the mock defense that ideas kept repeating and burned meaningful presentation time. That must not reach the paper. Each fact, stat, definition, and framing gets **one home** (its most relevant section); everywhere else references it, does not restate it. The reader/panel should never meet the same sentence or the same number twice.
   - **Page counts are budgets, not quotas.** A page target sizes how much *distinct* content a section should hold — never pad toward it by restating what was already said. If a section is short on real content, that is a content gap to fill or a target to lower, not a cue to repeat. Cutting a redundant paragraph always beats keeping it to hit a number.
   - **Where things live:** stats and their citations belong in the section that first needs them (usually Background/RRL); definitions belong in Definition of Terms; the mechanism/"why" is explained once. Later sections invoke the conclusion by name without re-quoting the figure or re-walking the explanation.
   - **Watch the classic overlaps:** Background vs. Statement of the Problem; Objectives vs. Scope; Significance vs. Conclusion; and every chapter draft vs. the proposal-presentation deck (pull from the draft, don't re-explain). When two sections cover the same ground, keep the evidence in one and let the other point to it.