...
| `source_id` | string | The pristine render this crop derives from; a pristine crop is its own source |
...
*Note.* The three degradation-level fields record the parameters of the operator $D$ from Section 3.2.3, so each crop's deformation is reproducible and can be analyzed after the fact. The `source_id` field ties every degraded crop back to the pristine render it came from, which is what the grouped split of Section 4.3.2 keys on.
