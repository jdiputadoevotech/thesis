...
1. Accept an uploaded generative-AI image in the raster formats admitted by Section 4.2.4 (PNG, JPEG, WebP) through the browser.
2. Localize the text at word granularity using an off-the-shelf text detector, cutting one crop per word.
3. Preprocess each crop and embed it with the frozen DINOv2 encoder and the trained metric head.
4. Check each crop for font homogeneity from the encoder's patch tokens, splitting a flagged crop once at the token-cluster boundary and matching each half on its own.
5. Apply the open-set decision to each crop, either ranking the localized palette, rejecting the crop as "unknown", or reporting it as "mixed typography".
6. Return a Top-K shortlist of candidate Google Fonts or the verdict that stands in its place for every result region, merging adjacent word crops that share a verdict into one region.
7. Render a preview of each candidate font so the user can compare it against the source text.
8. Expose the recognition pipeline through a REST endpoint (`/predict`) that the frontend calls.
9. Load the trained model weights once at startup and reuse them across requests.
...
7. Report an honest "unknown" or "mixed typography" rather than a forced guess when a crop fails the match test or the homogeneity check (transparency).
...
![Layered architecture diagram with a user tier (graphic designer), a client tier (React frontend), an API tier (FastAPI backend in Docker), an inference tier (PyTorch service running word-granularity text localization, preprocessing, the frozen DINOv2 encoder whose patch features also serve the homogeneity check, the metric head, and the open-set decision that ranks the palette, rejects unknowns, and flags mixed crops), and a resources tier (HuggingFace model store, offline-trained metric-head weights, and Google Fonts assets), with a request path down and a results path back up to the client carrying Top-K fonts, unknown, or mixed verdicts with rendered previews](../../../assets/figures/system_architecture.png)

As shown in Figure 9, a request flows top to bottom and the result returns to the client. The **client tier** is a React single-page application in the browser: it handles the image upload and renders the Top-K results gallery with font previews. It calls the **API tier**, a FastAPI backend packaged in a Docker image, which exposes the `/predict` endpoint, orchestrates the inference call, and loads the model weights when it starts. The API hands each request to the **inference tier**, a PyTorch service that runs the pipeline of Section 4.4: off-the-shelf text localization at word granularity, preprocessing, the frozen DINOv2 encoder whose patch tokens also serve the homogeneity check, the metric head $f_\theta$, and the open-set decision that produces a Top-K shortlist, an "unknown" verdict, or a "mixed typography" verdict per crop. The **resources tier** holds what the inference tier reads but does not compute at request time: the DINOv2 backbone and baseline weights pulled from the HuggingFace Hub, the metric-head weights trained offline, and the Google Fonts files used both as the recognition palette and to render the candidate previews.
