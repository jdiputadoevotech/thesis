...
- **Web app (built on top of model, for real-time testing) — carry through 4.5–4.8.** Introduced in 4.2.4 (testing instrument) and 4.4 (concept + operational logic). Downstream sections must cover it:
	...
	- 4.8 Software & Tools (tabular) — **DECIDED (2026-07):** Python/PyTorch (model), HuggingFace (DINOv2 + baseline weights), FastAPI (backend), React (frontend), Docker (deploy). Deploy scope = local demo harness (no auth/scaling/multi-user). Dev model = iterative-incremental (4 increments). Approach = bottom-up.
	- **DONE:** 4.5 (reqs + Fig 9 system architecture), 4.6 (iterative-incremental + Fig 10 SDLC), 4.7 (bottom-up approach), 4.8 (tools Table 2), 4.9 (schedule Fig 11 Gantt + responsibilities Table 3 + budget Table 4), 4.10 (V&V&T, build+model methods). Figs 9=`system_architecture.png`, 10=`sdlc_model.png`, 11=`gantt_timeline.png`. Tables: 1 (data schema, 4.3.1), 2 (tools, 4.8), 3 (responsibilities, 4.9.2), 4 (budget, 4.9.3).
	- **Ch4 methodology drafting COMPLETE** (4.1–4.10). Placeholders remaining: [N=100] panel size, [κ=X.XX] inter-rater, budget figures are planning estimates (labor rate ₱250/hr illustrative). Tool versions as-of Jul 2026 — verify at replication.
