...
Before training, each crop passes through a preprocessing chain: square-pad, resize to 224 × 224, and ImageNet normalization. This chain is embedded in the model's forward pass instead of being applied as a separate offline step. Folding preprocessing into the forward method, following Chen et al. (2026), guarantees that exactly the same transforms run at training time and at inference time. That eliminates train-serve skew (the class of bug in which a model is trained on inputs prepared one way and served inputs prepared another) when the model is later called from the web application. Degradation via the operator $D$ is applied once, when the corpus is built, rather than resampled each epoch. Kerning jitter is what forces this: it displaces glyphs by their advance widths, which exist only while the word is being typeset and cannot be recovered from a rasterized crop. Applying all three components at render time keeps the operator whole, and it matches what the rest of the design already assumes, since the per-crop intensities of Table 1 and the stratification by degradation level below both require each crop to carry one fixed deformation. Coverage of the deformation distribution comes from the breadth of the corpus instead: each font is rendered across the full range of intensities, so the model still meets deformation as a distribution. Following the augmentation-as-coverage argument of Plastropoulos and Tegos (2024), this pushes the model's decision boundaries into regions that stay stable under distortion.
