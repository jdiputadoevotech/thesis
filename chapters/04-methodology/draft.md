...
Two validation instruments follow from the word-level design of Section 4.4.2. First, the mixed-typography verdict is validated against a **mixed-font stressor set** the synthetic pipeline renders on demand: lines and words deliberately set in two palette fonts at controlled mixing ratios, degraded by the operator $D$ like every other crop, so the detection rate and false-alarm rate of the homogeneity check are measured on labeled data that no in-the-wild collection could supply. Because the served unit is the word crop, the threshold calibration of this section is likewise performed on word-granularity validation crops. Second, the choice of encoder is validated by a **backbone comparison** rather than assumed: the same frozen-encoder-plus-metric-head recipe, corpus, and calibration procedure are re-run with the backbone swapped — the incumbent DINOv2 against a contrastively pretrained vision-language encoder, a supervised ViT, and a convolutional baseline — holding everything else fixed, and each candidate is reported with its Top-1/Top-3 accuracy, false-positive rate at 95% recall, parameter count, and CPU and GPU inference latency. The homogeneity check narrows the field before speed and accuracy decide it: the check reads per-patch tokens, so a qualifying backbone must expose patch-level features, which is a property of the ViT family that a pooled convolutional embedding does not natively provide. The comparison therefore selects the deployed model on measured evidence, with the architecture class justified by the capability the pipeline requires of it.
